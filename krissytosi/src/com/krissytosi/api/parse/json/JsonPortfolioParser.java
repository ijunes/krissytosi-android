...
public class JsonPortfolioParser implements PortfolioParser {
    ...
    public List<Portfolio> parsePortfolios(String response) {
        ...
        if (response != null && !"".equalsIgnoreCase(response)) {
            ...
            if (response.length() != ApiConstants.HTTP_RESPONSE_CODE_LENGTH) {
                ...
                portfolios.add(createErrorPortfolio(ApiConstants.API_ERROR));
                Log.d(LOG_TAG,
    ...
    private Portfolio digestPortfolio(JSONObject portfolioJson) throws JSONException {
        ...
        if (!portfolioJson.has(ApiConstants.ERROR_IDENTIFIER)) {
            portfolio.setName(portfolioJson.getString(ApiConstants.NAME_ID));
            portfolio.setNumberOfImages(portfolioJson.getInt(ApiConstants.NUMBER_OF_IMAGES_ID));
            portfolio.setStartIndex(portfolioJson.getInt(ApiConstants.START_INDEX_ID));
            portfolio.setOrderIndex(portfolioJson.getInt(ApiConstants.ORDER_INDEX_ID));
        } else {
            portfolio = digestErrorResponse(portfolioJson
    ...
    private Portfolio digestErrorResponse(JSONObject errorJson) throws JSONException {
        ...
        errorPortfolio.setErrorCode(errorJson.getInt(ApiConstants.ERROR_CODE));
        errorPortfolio.setErrorDescription(errorJson.getString(ApiConstants.ERROR_DESCRIPTION));
    ...
    private Portfolio createErrorPortfolio(int errorCode) {
        ...
        errorPortfolio.setErrorDescription(ApiConstants.NO_PORTFOLIOS_DESCRIPTION);
        return errorPortfolio;
    }
}
